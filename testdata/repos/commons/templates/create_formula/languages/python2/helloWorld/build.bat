...
:build
    call :checkCommand python2
    call :checkCommand pip2
    ...
    CALL :SH_LINUX
    CALL :CP_DOCKER
    pip2 install -r %BIN_FOLDER%/requirements.txt
    GOTO DONE
...
:SH_LINUX
    ...
    echo pip2 install -r "$(dirname "$0")"/requirements.txt ^>^> /dev/null >> %SH_FILE%
    echo fi >> %SH_FILE%
    echo python2 "$(dirname "$0")"/main.py >> %SH_FILE%
    GOTO DONE
