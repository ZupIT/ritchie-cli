...
func buildCommands() *cobra.Command {
	...
	credSetter := credsingle.NewSetter(ritchieHomeDir, ctxFinder, sessionManager, dirCreater, fileManager)
	credFinder := credsingle.NewFinder(ritchieHomeDir, ctxFinder, sessionManager, fileManager)
	treeManager := formula.NewTreeManager(ritchieHomeDir, repoManager, api.SingleCoreCmds, fileManager)
